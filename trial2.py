...
loss_file = open("with_distillation1_1.txt", "w")
...
def validate(epoch):
    ...
    for (iter, image_x), (iter1, image_y) in  zip(enumerate(valid_loaderX), enumerate(valid_loaderY)):
        ...
        vutils.save_image(to_save, "results_val_model1_1/"+str(epoch)+"_"+str(iter)+".png" , normalize = True)
        to_save = torch.cat([out2, image_x, image_y], dim = 3)
        vutils.save_image(to_save, "results_val_model2_1/"+str(epoch)+"_"+str(iter)+".png" , normalize = True)
...
if(args.mode == "train"):
    ...
    for epoch in range(0, 500):
        ...
        for (iter, image_x), (iter1, image_y) in  zip(enumerate(train_loaderX), enumerate(train_loaderY)):
            ...
            vutils.save_image(to_save1, "results_train_model1_1/"+str(epoch)+"_"+str(iter)+".png" , normalize = True)
            to_save2 = torch.cat([dec_out5y, image_x, image_y], dim = 3)
            vutils.save_image(to_save2, "results_train_model2_1/"+str(epoch)+"_"+str(iter)+".png" , normalize = True)

        checkpoint_en = {'model': encoder_decoder() ,'state_dict': net1.state_dict(), 'optimizer' : optim1.state_dict()}
        torch.save(checkpoint_en, 'model1_1.pth')
        checkpoint_en = {'model': encoder_decoder() ,'state_dict': net2.state_dict(), 'optimizer' : optim2.state_dict()}
        torch.save(checkpoint_en, 'model2_1.pth')
        validate(epoch)

elif(args.mode == "test"):

    net1 = load_checkpoint('model1_1.pth').to(device)
    net2 = load_checkpoint('model2_1.pth').to(device)
    ...
    save_dir = "/home/ruchika/GNR638Project/RESULTS_ON_DEMO/outputs_AE2/"
    imageSize = (64, 64)
    normalize = transforms.Normalize((0.1307,), (0.3081,))
    transform = transforms.Compose([transforms.Scale(imageSize), transforms.ToTensor(), normalize,])
